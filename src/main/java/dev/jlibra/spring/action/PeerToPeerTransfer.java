    ...
    protected long fetchLatestSequenceNumber(byte[] address) {
        ...
        return result.getAccountStates()
                ...
                .filter(accountState -> Arrays.equals(
                        accountState.getAccountAddress(), //修改过 原本是 accountState.getAccount()
                        address))
                .map(AccountData::getSequenceNumber) //修改过 原本是 AccountState::getSequenceNumber
                .findFirst()
                .orElse(0);
    }
...
//                    && result.getMempoolStatus() == MempoolAddTransactionStatus.Valid) {
            if (result.getAdmissionControlStatus().getCodeValue() == 0
                    && result.getMempoolStatus().getCodeValue() == 0) {
